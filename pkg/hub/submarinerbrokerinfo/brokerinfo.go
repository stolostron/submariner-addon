...
func applyGlobalnetConfig(client client.Client, brokerNamespace, clusterName string,
	brokerInfo *SubmarinerBrokerInfo, submarinerConfig *configv1alpha1.SubmarinerConfig,
) error {
	gnInfo, _, err := globalnet.GetGlobalNetworks(client, brokerNamespace)
	...
	if gnInfo != nil && gnInfo.Enabled {
		...
		err = globalnet.AllocateAndUpdateGlobalCIDRConfigMap(client, brokerNamespace, &netconfig, status)
		if err != nil {
