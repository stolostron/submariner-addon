...
func TestWrapManifestWorks(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if err == nil && !c.expectErr {
				works, err := fakeWorkClient.WorkV1().ManifestWorks(c.clusterName).List(context.TODO(), metav1.ListOptions{})
				if err != nil {
					if errors.IsNotFound(err) && c.expectWorkNum != 0 {
						t.Errorf("expect %v manifestworks,but get 0 ", c.expectWorkNum)
					}
					t.Errorf("failed to get manifestworks %+v", err)
				}
				if len(works.Items) != c.expectWorkNum {
					t.Errorf("expect %v manifestworks,but get %v ", c.expectWorkNum, len(works.Items))
				}
			}
		})
	}
